...
merged = election_data_1.merge(election_data_2, on='title')
merged.to.csv("election_dump_master.csv", index=False)
...
print (sum) 
#the count of voting done
#TotalVotes = str.count("election_dump_master.csv", start = 0, end=len(string))
    print(TotalVotes)

#count of voting and percentage by candidates
